...
vi.mock("@/features/silpo/demo/demo-gateway", () => ({
  createDemoSilpoGateway: vi.fn(),
}));
...
describe("GET /api/backtest", () => {
  ...
  it("returns 200 in demo mode with report and no-store headers", async () => {
    ...
    vi.mocked(createDemoSilpoGateway).mockReturnValue(mockGateway);
    ...
    expect(createDemoSilpoGateway).toHaveBeenCalledTimes(1);
  ...
  it("handles factory or service throw with safe 500 and masks secret sentinel", async () => {
    ...
    vi.mocked(createDemoSilpoGateway).mockImplementation(() => {
